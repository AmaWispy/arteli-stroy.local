(function () {
    ...
    const renderQuestion = (index = 0) => {
        ...
        question.innerHTML = `
            ...
            } class="quiz__question-title">${questions[index].question}</div>
    ...
    const renderProgress = (currentStep) => {
        ...
        currentProgressBar.classList.add(
            "quiz-indicator__current-progress-bar"
        );
        currentProgressBar.style.width =
            (100 * currentStep) / questions.length + "%";
    ...
    quiz.addEventListener("change", (e) => {
        if (
            ...
            e.target.getAttribute("data-is-textarea")
        ) {
            return;
        }
        ...
        setTimeout(() => {
            ...
            const obj = {
                [`${questions[nextQuestionIndex - 1].question}`]: `${
                    questions[nextQuestionIndex - 1].answers[e.target.value]
                        .value
                }`,
            };
            results.push(obj);
    ...
    quiz.addEventListener("input", (e) => {
        if (
            e.target.getAttribute("type") !== "text" ||
            e.target.getAttribute("data-form")
        ) {
            return;
        }
