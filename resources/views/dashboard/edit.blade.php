<x-dashboard.layout>
  <form class="w-100 mx-auto" action="/update-article" method="post">
    @csrf
    ...
    <input type="hidden" name="id" value="{{ $article->id }}">
      ...
      <div class="mb-5">
          ...
          <input type="text" id="h1" name="h1"
                 ...
                 placeholder="H1 заголовок" value="{{ $article->h1 }}"/>
          @error('h1')
      ...
      @if (auth()->user()->role === 'admin')
          <div class="mb-5">
              ...
              <input type="text" id="link" name="link"
                     class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                     placeholder="Ссылка на статью" value="{{ $article->link }}"/>
              @error('link')
              <p class="text-red-700">{{$message}}</p>
              @enderror
          </div>
      ...
      <div class="mb-5">
          ...
          <input type="text" id="short_title" name="short_title"
                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                 placeholder="Короткий заголовок" value="{{ $article->short_title }}"/>
          @error('short_title')
          <p class="text-red-700">{{$message}}</p>
          @enderror
      </div>
      ...
      <div class="mb-5">
          ...
          <textarea id="text" name="text" rows="10"
                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 "
                    placeholder="параграф">{{ $article->text }}</textarea>
          @error('text')
          <p class="text-red-700">{{$message}}</p>
          @enderror
      </div>
          ...
          <div class="media-field">
              <div class="hidden">
                  <input type="text" id="image" name="image"
                         class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                         placeholder="img/pages/...promo-300x.webp" value="{{ $article->image }}"/>
              </div>
              <div class="picture">
                  <div class="flex preview small">
                      <img class="shadow rounded" src="{{ url($article->image) }}">
                  </div>
                  <button class="btn-pick text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center my-2">
                      Выбрать изображение
                  </button>
                  <a class="btn-original text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center my-2"
                     href="{{ url($article->image) }}" target="_blank">Оригинал</a>
              </div>
          </div>
          ...
          <div class="media-field">
              <div class="hidden">
                  <input type="text" id="img_big" name="img_big"
                         class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                         placeholder="img/pages/...promo.webp" value="{{ $article->img_big }}"/>
              </div>
              <div class="picture">
                  <div class="flex preview">
                      <img class="shadow rounded" src="{{ url($article->img_big) }}">
                  </div>
                  <button class="btn-pick text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center my-2">
                      Выбрать изображение
                  </button>
                  <a class="btn-original text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center my-2"
                     href="{{ url($article->img_big) }}" target="_blank">Оригинал</a>
              </div>
          </div>
          ...
          <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 ">
              <div class="px-4 py-2 bg-white rounded-b-lg ">
                  ...
                  <textarea id="content" name="content" rows="50"
                            ...
                            placeholder="Контент">{{ $article->content }}</textarea>
                  @error('content')
                  <p class="text-red-700">{{$message}}</p>
                  @enderror
              </div>
      ...
      <div class="mb-5">
          ...
          <select id="author_id" name="author_id"
              ...
              <option value="{{ $author->id }}"
                      {{ $author->id === $article->author_id ? "selected" : "" }}
              >
                  {{ $author->name }}
              </option>
              @php
    ...
    <div id="confirm-deletion" tabindex="-1"
        ...
        <div id="modal" class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form class="p-4 md:p-5 text-center" method="post" action="/delete-article">
                    @csrf
